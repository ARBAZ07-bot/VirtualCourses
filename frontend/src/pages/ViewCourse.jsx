...
const avgRating = calculateAverageRating(selectedCourseData?.reviews);
  ...
  const fetchCourseData = () => {
    const match = courseData.find((item) => item._id === courseId)
    if (match) {
      dispatch(setSelectedCourseData(match))
    }
  }
    ...
    <div className="mb-4">
      ...
      >
        ...
        <div className='w-full transition-all duration-300 py-[20px]   flex items-start justify-center lg:justify-start flex-wrap gap-6 lg:px-[80px] '>
            ...
            {
                selectedCreatorCourse?.map((item)=>(
                    <Card key={item._id} thumbnail={item.thumbnail} title={item.title} id={item._id} price={item.price} category={item.category}/>
                ))
            }
        </div>
      </div>
