    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="w-full transition-all duration-300 py-[130px] md:pl-[300px]  flex items-start justify-center md:justify-start flex-wrap gap-6 px-[10px]">
        ...
        filterCourses?.map((item)=>(
          <Card key={item._id} thumbnail={item.thumbnail} title={item.title} price={item.price} category={item.category} id={item._id} reviews={item.reviews} />

        ))
      }
      {filterCourses?.length === 0 && (
        <p className="text-gray-500 text-center mt-10 w-full">No courses found.</p>
      )}
      </main>
    </div>
  );
